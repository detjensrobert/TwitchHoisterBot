...
async function execute(message) {
	...
	commands.forEach((cmd) => {
		...
		if (!cmd.roleRestrict || (cmd.roleRestrict && message.guild && message.member.roles.cache.has(config.roles[`${cmd.roleRestrict}`]))) {
			...
			if (cmd.roleRestrict) {
				...
				helpStr += `\n*(Restricted to @${ message.guild.roles.cache.get(roleID).name } only)*`;
			}
